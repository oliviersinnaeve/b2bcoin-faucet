...
class B2BFaucet {
    ...
    protected $api_key;
    protected $currency;
    // protected $api_base = "http://api.java-developer.be/b2bcoin/api/faucet/";
    // protected $fiab_api_base = "http://api.java-developer.be/b2bcoin/api/faucet/";
    protected $api_base = "https://api.b2bcoin.xyz/b2bcoin/api/faucet/";
    protected $fiab_api_base = "https://api.b2bcoin.xyz/b2bcoin/api/faucet/";
    public $communication_error = true;
    ...
    public function __execPHP($url, $params = array()) {
        error_log("PHP Method: ".$url);
        error_log("PHP Parameters : ".implode(",",$params));
        error_log("PHP cacert location: ".dirname(__FILE__) . '/cacert.pem');
        ...
        $opts = array(
            ...
            //     "verify_peer_name" => $this->options['verify_peer_name']
            // )
        );
    ...
    public function __execCURL($url, $params = array()) {
        ...
        error_log("CURL Parameters : ".implode(",",$params));
        $params = array_merge($params, array("api_key" => $this->api_key, "currency" => $this->currency));
    ...
    public function send($to, $amount, $referral = "false") {
        ...
        error_log("SEND Response : ".implode(",", $r));
        
        if (is_array($r) && array_key_exists("status", $r) && $r["status"] == 200) {
            return array(
                'success' => $r["success"],
                'message' => $r["message"],
                // 'message' => 'Payment sent to you using B2BCoin.xyz',
