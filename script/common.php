...
$db_updates = array(
    ...
    66 => [
        ...
        "INSERT IGNORE INTO `Faucetinabox_Settings` (`name`, `value`) VALUES ('service', 'b2bcoin');",
...
create table if not exists Faucetinabox_Refs (
    ...
    `address` varchar(96) not null unique,
    primary key(`id`)
);
...
create table if not exists `Faucetinabox_Address_Locks` (
    `address` varchar(96) not null primary key,
    `locked_since` timestamp not null
);
...
('name', 'B2BCoin.xyz'),
('short', 'Just another B2BCoin.xyz faucet :)'),
('template', 'default'),
